...
export default function AdminCoursesPage() {
  ...
  if (authLoading) return <div className="min-h-screen bg-[#0f172a] flex items-center justify-center text-amber-500">Checking Access...</div>;
  ...
  return (
    <div style={layoutStyle}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflowY: 'auto' }}>
        <header style={{ height: '70px', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', padding: '0 30px', background: '#1e293b' }}>
            <h1 className="font-bold text-lg tracking-wider text-white">Course Management</h1>
            <div style={{ marginLeft: 'auto', background: '#334155', padding: '5px 12px', borderRadius: '4px', fontSize: '12px' }}>
                Total Courses: <span className="font-bold text-white ml-2">{courses.length}</span>
            </div>
        </header>
            ...
            <div className="bg-[#1e293b] border border-slate-700 rounded-xl overflow-hidden shadow-xl">
                <table className="w-full text-left text-sm text-slate-400">
                    ...
                    <tbody className="divide-y divide-slate-700">
                        {loading ? (
                             <tr key="loading"><td colSpan="4" className="px-6 py-8 text-center text-slate-600">Loading courses...</td></tr>
                        ) : courses.length === 0 ? (
                             <tr key="empty"><td colSpan="4" className="px-6 py-8 text-center text-slate-600">No courses found.</td></tr>
                        ) : (
                            courses.map((course, index) => (
                                <tr key={course.id || course.course_code || index} className="hover:bg-slate-700/30 transition-colors">
                                    <td className="px-6 py-4 font-mono text-blue-400">{course.course_code || course.code}</td>
